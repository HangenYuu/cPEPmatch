#!/opt/conda/envs/cpepmatch/bin/python
# # cPEPmatch
...
def main(args):
    ...
    database = cpep_database.create_cyclolib_database(
        args.database_location,
        args.motif_size,
        args.consecutive,
        args.cyclization_type,
        args.exclude_non_standard,
    ...
    interface_residues = protein_target_characterization.create_interfaces(
        args.working_location,
        args.protein,
        args.target,
        args.pdb_name,
        args.interface_cutoff,
        residues=args.protein_specific_residues,
    )
    protein_motifs = protein_target_characterization.process_interface(
        args.working_location,
        args.pdb_name,
        args.motif_size,
        motif_type=args.consecutive,
    )
    update_progress(0.5)
    print("\nStep 2 complete.\n")
    ...
    # 3. Find Matches
    all_match = backbone_match.find_match(
        args.database_location,
        protein_motifs,
        args.motif_size,
        args.consecutive,
        args.frmsd_threshold,
    )
    all_match.sort()
    ...
    with open("md.log", "w") as f:
        sys.stdout = f
        real_match_list = superimpose_mutate.superimpose_and_mutate(
            args.working_location,
            all_match,
            args.motif_size,
            args.interface_cutoff,
    ...
    print(
        "\n \nYour results are now available in the following directory: {}".format(
            args.working_location
        )
    )
...
if __name__ == "__main__":
    ...
    parser = argparse.ArgumentParser(
        description="cPEPmatch: Python program designed to find cyclic peptides that mimic proteins and target their binding partners."
    ...
    parser.add_argument(
        ...
        "--pdb_name",
        default="system",
        help="PDB file name, should contain both the protein and the target",
    )
    parser.add_argument(
        "-p", "--protein", default="A", help="Protein to mimic chain name"
    )
    parser.add_argument("-t", "--target", default="B", help="Target chain name")
    parser.add_argument(
        "-wl",
        "--working_location",
        default=default_test_system_location,
        help="Path to where your protein-target pdb file is located. Results will be outputted here",
    )
    parser.add_argument(
        "-dl",
        "--database_location",
        default=default_database_location,
        help="Path to where the cPEPmatch database is located",
    )
    ...
    parser.add_argument(
        ...
        type=int,
        default=6,
        help="Distance in angstroms to define the contact interface residues that will be matched",
    )
    parser.add_argument(
        "-ft",
        "--frmsd_threshold",
        type=float,
        default=0.5,
        help="Fit-RMSD value in angstroms of how precise you want the matching",
    )
    parser.add_argument(
        "-ms", "--motif_size", type=int, default=5, help="Number of CA carbons to match"
    )
    parser.add_argument(
        "-cs",
        "--consecutive",
        ...
        default=True,
        help="Motif type: consecutive or non-consecutive motifs",
    )
    parser.add_argument(
        "-psr",
        "--protein_specific_residues",
        default="",
        help="Selection of given residue numbers to match, for example, if hot-spot analysis has been done prior to the matching",
    )
    ...
    parser.add_argument(
        ...
        "--cyclization_type",
        default="",
        help="Select only cyclic peptides from the database that are cyclized in a specific type, e.g., 'head to tail'",
    )
    parser.add_argument(
        "-ens",
        ...
        type=lambda x: (str(x).lower() == "true"),
        default=False,
        help="Exclude cyclic peptides from the database containing non-standard amino acids.",
    )
